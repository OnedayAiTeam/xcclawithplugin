...
2. **LonglinkHub** — `ws` client, reconnect on close, parses downlink frames, applies `user_dm_ok` / `peer_message_ok` into **in-memory** maps.
...
5. **Tools** — `xcclawith_directory` (HTTP directory), `xcclawith_peer_message` (longlink peer send).
